...
For overall strength rating, most covariates revealed linear associations (see Table \ref{tab:appendix:gam-overall-rating-reml} lists the coefficients). In Figure \ref{fig:personality:study2:rating-b5-predictors-G_Overall}, we see that participants who scored higher on the ``Openness'' trait generally judged passwords lower. This effect was flagged as significant in the model ($B=-0.36$). Although having a computer-science background revealed stronger correlation estimates ($B=-2.92$), the association was not significant. 
