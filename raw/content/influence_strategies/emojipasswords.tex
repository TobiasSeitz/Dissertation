...
% Do not require a big change to existing back-ends / authentication architectures
...
% context: visual authentication
% but: unique project: hybrid between alphanumeric knowledge-based passwords and recognition based pws / –combine– lexial memory AND visual memory. 
...
\fbox{
	...
	\parbox[c][8cm]{0.7\linewidth}{
		\section*{Take Aways}
		\begin{itemize}[leftmargin=*]
			\item Users' intuitive reaction to creating an emoji password was to try to use established strategies, with memorability in mind. 
		\end{itemize}
	}
	\hspace{1cm}
}
