...
Instead of users creating new passwords, it is possible to make inferences about behavior from already existing data. Password frequency lists are readily available on the Internet \footnote{Example repository containing a wide range of leaked passwords available under \url{https://github.com/danielmiessler/SecLists/tree/master/Passwords}\la{09.01.2018}}. The sources can often be traced back to illegal attacks, which makes the use of the data somewhat questionable. However, it is a widely accepted method to contrast real-world and study behavior. The dataset that has probably been studied the most originates from a breach at RockYou, a software development firm specialized at games for social networks. For instance, Bonneau 
...
Moslty quantitative, automated retrospective analyses (common leaks see Table \ref{tab:rw:password_leaks}).
