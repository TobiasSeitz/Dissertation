...
If we cannot collect the users' real passwords in its original form, what is the best way to measure, e.g., cause and effect of interventions? There are several alternatives. First, one asks participants to create a new password during the user study and stores these passwords as part of the dataset. This approach resolves the issue of real-world password disclosure, but introduces a number of problems.
...
Some users, however, are less suspicious and provide one of their real passwords regardless of the instruction. The other alternative would be to conceal the purpose of the study through an act of deception, which is occasionally done in psychology studies (for a discussion see \cite{Tai2012Deception}). The data would be ecologically valid because it removes observer-expectancy and other biases. For the researchers it is extremely difficult to tell ``real'' passwords and ``new'' passwords apart. Scientific research should also be valid, reliable, and generalizable. Therefore, the dataset is often part of the publication to allow others to assess these three qualities. But if users had entered their real passwords, it is risky to publish this dataset and leads to a second ethical conundrum. 
...
% solving the ``plain text password problem'': scenarios, IRBs, encryption, do not release dataset, differential privacy -- latest bonneau research


% second solution: do not collect the plain text password
A second alternative to collect password data through user studies is to refrain from storing the original password, but to transfer the password into a pseudonymous representation. For instance, Von Zezschwitz \etal used a ``meta password'' that described the participants' actual passwords \cite{VonZezschwitz2013SurvivalShortest}, but which was insufficient to reconstruct the original. To collect the data, participants were provided with an offline password analysis tool. They entered their password into this tool and copy-pasted the result of the analysis into the questionnaire form. 
...
%TODO: mental rotation tests Peters, M. Revised Vandenberg & Kuse Mental Rotations Tests: forms MRT-A to MRT-D. Technical Report, Department of Psychology, University of Guelph, 1995
...
password lists are readily available on the internet, e.g. on a number of repositories\footurl{https://github.com/danielmiessler/SecLists/tree/master/Passwords}{03.01.2018} on GitHub
