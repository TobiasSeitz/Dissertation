...
Table \ref{table:rw:benefits_drawbacks_pws} shows a high-level summary of the benefits and drawbacks that come along with password-based authentication. We can observe that the drawbacks constitute important constraints and it is natural to try to remove such limitations by considering alternatives. This section briefly covers the most notable approaches to authenticate users without alphanumerical passwords or with additional security measures.
	...
	distinction between graphical, visual, and gestural authentication
	
	Most notably, graphical passwords have seen widespread adoption when Android introduced a locking mechanisms that prompts the user to draw a pattern instead of entering a PIN \cite{Biddle2009GraphicalFirstTwelveYears}. 
	
	
	locimetric, drawmetric. 

\todo{add a representative image with three different types of graphical authentication tools}

%THeme: clicking images in the right order: recognition based?
Dhamija and Perrig created ``Deja Vu'' \cite{Dhamija2000DejaVu}

Hayashi \etal blurred images which only the legitimate user could identify during authentication \cite{Hayashi2008UseYourIllusion}

Bicakci explored authenticating through clicking a sequence of icons including a password manager to facilitate authentication for the users \cite{Bicakci2011PWMIconBased}.

Graphics as additional layer \cite{Gao2010ColorLogin} \cite{DeLuca2010ColorPIN}

De Angeli VIP something like pass-faces. \cite{DeAngeli2002VIP}

Golla \etal \cite{Golla2017EmojiAuth}

Dunphy closer look at recognition based \cite{Dunphy2010CloserLookGraphical}

Brostoff and Sasse evaluated the commercial solution ``Passfaces'' \cite{Brostoff2000PassfacesEvaluation}

%theme: clickig points in the right order
Chiasson \etal presented the Cued Click Point (CCP) scheme \cite{Chiasson2007CCP}, and the Persuasive Cued Click Point scheme \cite{Chiasson2008PCCP} (CCP). To authenticate, the user has to click a certain number of distinct points inside an image. The PCCP version tries to nudge the user to pick less predictable locations, which is supposed to increase the ``strength'' of the graphical password. 

%theme: (visualized) gestures (on mobiles)
...
%benefits 
A number of findings showed benefits of graphical authentication. For instance, Chiasson \etal were report on a lab study which showed that graphical passwords are easier to remember and less error-prone in the short term \cite{Chiasson2009InterferencesGraphical}. In the long term, however, there was not significant difference to textual passwords. Imran found that graphical passwords are easily understood by children \cite{Imran2015PWsAdultsChildren}.

Carter presented a new system specifically designed for older adults \cite{Carter2015GraphicalPasswordsOlderUsers}

%psychological / cognitive background info
Paivio \etal investigated why pictures are easier to recall than words \cite{Paivio1968PicturesEasierThanWords} (\textit{picture superiority effect}), Grady \etal say this works mostly if there is an episode that serves as mnemonic \cite{Grady1998NeuralCorrelates} 

Elizabeth Stobert dedicated a large part of her PhD work to graphical passwords \cite{Stobert2015Dissertation}. She found memorability benefits \cite{Stobert2013GraphicalPasswords}

%user strategies, real world behavior = drawbacks
...
\todo{more shoulder surfing drawbacks, not caught on in the real world, maybe emoji stuff}
...
De Angeli \etal doubted the benefits early on \cite{DeAngeli2005PictureThousandWords}  and together with Renaud they kept on criticizing visual authentication \cite{Renaud2009VisualSnakeOil}, 
	...
	Anecdote to show that it is a bad
	 ...
	 idea \footurl{https://www.theguardian.com/world/2017/nov/08/qatar-airways-plane-forced-to-land-after-wife-discovers-husbands-affair-midflight}{28.12.2017} 
