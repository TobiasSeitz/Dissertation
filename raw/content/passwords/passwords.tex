...
To obtain the password of a specific user, one can also simply watch them enter it and figure out what the password was. This attack is typically referred to as ``shoulder surfing'' because the person who watches figuratively ``surfs'' on the ``shoulder'' of the user to look at their screen \cite{Tari2006ShoulderSurfingComparison}. Shoulder surfing is relatively easy and does not require technical sophistication. But, although friends and family can carry out such an attack, it is safe to say that it is more problematic in public spaces where unknown bystanders are present. Typical authentication that happens in such environments can be found with PIN entry, e.g. on an ATM, or entering any kind of password on a mobile device, e.g. on a bus in close vicinity to other people. De Luca \etal dedicated some research to both scenarios. \cite{DeLuca2010ColorPIN, DeLuca2010UnderstandingATMSecurity}
...
There isn't a lot of evidence that this is a severe threat in the real world, attack is non-scalable \cite{Herley2015Counterfactuals} \cite{Herley2015Counterfactuals}. 

typically graphical password schemes are more concerned with this attack, because the screen real estate needed to display and enter them is usually higher.  \cite{Tari2006ShoulderSurfingComparison}  \cite{Wiedenbeck2006ConvexHull}
...
\cite{DeLuca2013FakeCursors}\cite{Roalter2013SmartphoneProxy}
