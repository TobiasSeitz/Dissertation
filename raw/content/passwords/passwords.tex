...
%	Graphics as additional layer \cite{Gao2010ColorLogin} \cite{DeLuca2010ColorPIN}
	...
	keystrokes (with explicit enrollment):  on a smartphone \cite{Buchoux2008KeystrokeAnalysis} (given it was 2008, this is really early). Bouchoux state the explicit enrollment might be too bothersome, so it is possible to enroll the user implicitly, with the risk of including data from impostors, too. Web based mobile applications with Trusted Third Party, i.e. a federated approach \cite{Nauman2010TOKEN} requires TPM. 
...
% privacy could be an issue for some users (where is my data stored)
