...
% User perspective
Users are challenged to find a password that withstands this kind of attack. The large issue is that attackers are basically only limited by the time they want to spend calculating password hashes. On modern machines with a single GPU, thousands of hashes can be calculated per second even for slow algorithms\footurl{https://gist.github.com/epixoip/9d9b943fd580ff6bfa80e48a0e77520d}{20.12.2017}. This is why Florêncio \etal argue that it is futile to encourage users to pick a password that would withstand such an attack \cite{Florencio2014AdministratorsGuide, Florencio2016CommACM}. 

%% PHISHING WEBSITE SCREENSHOT
\begin{figure}
	\centering
	\includegraphics[width=0.8\linewidth]{rw/facebook-phishing-site}
	\caption{\label{fig:rw:phishingsite} Actual phishing website targeting the facebook login that was accessible on 21.12.2017. The URL contains ``fb'' which might mislead users into thinking this is a legitimate facebook URL. }
\end{figure}
...
It is very challenging for users to validate the authenticity of a given webpage \cite{Dhamija2006WhyPhishingWorks, Fogg2001WhatMakesSitesCredible}. A lot of research has been dedicated to helping users in this validation and prevent phishing attacks. For example, Lin \etal found that domain highlighting and instructing users on it only has a small effect on the effectiveness of phishing attacks \cite{Lin2011DomainHighlighting}. Wu \etal showed that toolbars do not really help users, either \cite{Wu2006SecurityToolbars}. Dhamija \etal proposed a \textit{trusted path} between the user and the legitimate service \cite{Dhamija2005DynamicSecuritySkins}. Users were supposed to verify the authenticity by comparing visual patterns in a trusted window and the website. Together with Max-Emanual Maurer and Alexander De Luca, I created a browser extension to visualize the usage of different types of SSL certificates across websites through the entire browser skin \cite{Maurer2011ShiningChrome}. We deployed it publicly and launched a feedback survey, which indicated that changing the browser skins is obtrusive enough to raise awareness and makes users more confident while browsing the Web. However, optimally the browser would detect phishing websites and prevent that users visit them in the first place. Current versions of the major browsers try to do this and urge the user to leave the site, as is shown in Figure \ref{fig:rw:browser_warning}.
...
%Important because lots of financial loss and implications for password reuse. And we have a paper on this \cite{Maurer2011ShiningChrome}
