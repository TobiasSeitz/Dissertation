...
Moreover, while ``\texttt{\{X@T;cuXw[bJnUH}'' is a randomly generated password that seems difficult to guess, it does not necessarily have to be a \textit{strong} password (see Section \ref{sec:rw:whats_a_bad_pw}): if it was found in a password database after a data breach, it is likely to be included among the first guesses in subsequent dictionary attacks. The number of leaked passwords has steadily increased in the past few years and thus has rendered many such strong passwords unsuitable (see footnote \ref{foot:rw:breachlevelindex}\todo{make this work}). In the same vein, Yang \etal state: ``If a strategy is widely used, then attackers may develop strategy-specific methods which can efficiently guess the passwords'' \cite{Yang2016MnemonicSentenceBased}. Economists call this the ``Tragedy of the Commons'' \cite{Hardin1968TragedyCommons}. This, for example, implies that if passphrases (cf. Section \ref{sec:rw:passphrases}) do become commonplace, attackers will optimize their attack strategy rendering the well-intended efforts to strengthen passwords less effective. Consequently, the challenge of considering such effects and realistically measuring password strength has sparked some discussion, which has led to a small set of suitable models which are discussed below.
	...
	\subsection{The zxcvbn Approach: Lightweight, Robust, Simple}
...
\begin{enumerate}\setcounter{enumi}{5}
	...
	\item \todo{get feedback: if people think the list is useful, summarize more of these points}
\end{enumerate}
...
Table \ref{table:rw:benefits_drawbacks_pws} shows a high-level summary of the benefits and drawbacks that come along with password-based authentication. We can observe that the drawbacks constitute important constraints and it is natural to try to remove such limitations by considering alternatives. To give the reader a sense of the alternatives, this section briefly covers the most notable approaches to authenticate users without alphanumerical passwords or with additional security measures. 
	...
	% IMAGES - SEARCHMETRIC
	\todo{reference to individual images in text.}
	\begin{figure}[h]
		\centering
...
%	Graphics as additional layer \cite{Gao2010ColorLogin} \cite{DeLuca2010ColorPIN}
	...
	\textbf{Fingerprints} are becoming\footurl{https://www.deloitte.co.uk/mobileuk/\#gold-finger-fingerprints-lead-biometric-authentication}{02.01.2018} the most common feature used biometric authentication method nowadays, due to the widespread device-support (mobiles phones, tablets, laptops) and obvious usability benefits as perceived by the users\cite{Wash2010FolkModels}. On modern phones, after a short enrollment, fingerprint recognition takes a fraction of a second and has low error rates under normal circumstances \cite{Henniger2016SecurityEvaluationFingerprint}. Thus, fingerprint authentication is fairly usable and reasonably secure. However, a wet surface, grease, or dirt can limit the functionality \cite{Bhagavatula2015BiometricPerceptions}. Moreover, attacks (like fingerprint spoofing with a 2D/3D print) are becoming easier to carry out, though they still require decent effort \cite{Marasco2014SurveyAntispoofing}. Recently, more systems are counting on \textbf{iris and facial recognition} to authenticate users.
%
%	fingerprint omnipresent 
%	iris scan
%
%	face recognition (iPhone X)
%	voice / speech ``Both face and voice recognition logins are extremely situational dependent; photos of faces don’t work in poor lighting and speaking into a microphone doesn’t always work in noisy environments.''\footurl{https://www.inauth.com/blog/fingerprints-popular-biometric/}{02.01.2018}
	
	% features that are used for implicit authentication
	\subsubsection{Implicit Biometrics}
	gait: first ones with accelerometers: Ailisto \etal \cite{Ailisto2005GaitPatternAccelerometers}, then with mobile phones (iPhone) \cite{Tamviruzzaman2009ePet}
	
	keystrokes (with explicit enrollment):  on a smartphone \cite{Buchoux2008KeystrokeAnalysis} (given it was 2008, this is really early), web based mobile applications with Trusted Third Party, i.e. a federated approach \cite{Nauman2010TOKEN} requires TPM. 
	
	usage behavior (filesystem and network): \cite{Yazji2009ImplicitReauthentication}
	
	overview: \cite{Jakobsson2009ImplicitAuthentication}
	
	\cite{Peisert2013PriciplesAuthentication,Rybnicek2014RoadmapContinuousAuth}
	
	multiple cues / multimodal: early work: \cite{Brunelli1995PersonIdentificationMultipleCues}, in 2005 \cite{Bigun2005CombiningBiometricEvidence}



	
	% attack vectors (briefly summarize in one sentence per paper.)
	Anecdote to show that it is a bad: \footurl{https://www.theguardian.com/world/2017/nov/08/qatar-airways-plane-forced-to-land-after-wife-discovers-husbands-affair-midflight}{28.12.2017} 
	...
	% implicit 
	% how does it technically work on (roughly)?

	
	% what can you do? what are the benefits?
	De Luca showed that users unlock their phones in a unique way, which can be leveraged for implicit authentication \cite{DeLuca2012TouchMeOnce}. Interestingly, at the time of writing this thesis, De Luca \etal's paper is the most-cited CHI publication in the past 5 years, which highlights the impact and potential of this approach. 
	
	Idea of continuous authentication \cite{Peisert2013PriciplesAuthentication}
	
	
	\paragraph{Issues}
	% biometrics are based on probability -- provide some metrics.
	...
	actually the technical report gives some insights about this \cite{Stockinger2011ImplicitAuthentication}, so that will inspire a bit. also look into the USEC folder in the office, the papers and annotations are in there.	
	...
	Take away: biometric authentication is nice, because it increases usability and reduces interaction times to unlock a system \ar, the weak point appear to be the fallback methods. Moreover, implicit authentication cannot replace explicit prompts, but it can make an existing scheme more secure. There's nothing stopping us from developing systems that at some point can read DNA.
...
SecureID or ``Gnubby'' (see Google)
	...
	% multifactor authentication: early idea:
...
If authentication mechanisms were a tournament, users would prefer OAuth and QR-Codes to authenticate, but in the real world, this isn't the case \cite{Ruoti2015AuthenticationMelee}
	...
	% wearable token:
	Jakobsson developed a bracelet that serves as secure token \cite{Jakobsson2014HowToWearYourPW}. This idea has had impact on actual products, because smart watches can now serve as authentication token: Google offers the ``Trusted Device'' feature since Android version XX \todo{find out.}. If a trusted device, e.g. a smart watch or wireless headphones, is connected to the phone, the phone turns of the primary authentication method, e.g. pattern unlock, while the device is connected. If the phone has been inactive for four hours, the user needs to re-authenticate by providing the primary credential. d
