...
### factorize things
d_comp100$D_Gender <- factor(d100$D_Gender,levels=c("Male","Female"),labels=c("Male","Female"))
d_comp100$D_ComputerScienceBackground <- factor(d100$D_ComputerScienceBackground, levels=c("No","Yes"), labels=c("No","Yes"))
...
autoModelsComparisonB5 <- lapply(responseVariablesComparison, getSmoothedGAM, predictors=predictorsB5, d<-d_comp100)
...
for(i in updatedModelsComparisonB5) {
  outputSummary(i, prefix="comparison-b5-", path="summaries")
}
